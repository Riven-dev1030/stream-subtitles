...
**文件版本**: 2026-03-08 (更新 Phase 3.2)
**相關代碼**: `extension/content/content.js`, `extension/background/claude-translator.js`
...
5. [上下文感知翻譯 (Phase 3.2)](#上下文感知翻譯-phase-32)
6. [術語表 (Glossary) 處理](#術語表-glossary-處理)
7. [定時清理機制](#定時清理機制)
8. [字數額度共享機制](#字數額度共享機制)
9. [時間參數配置](#時間參數配置)
10. [版本演進歷史](#版本演進歷史)
...
為了提升翻譯的自然度與一致性，系統在 Phase 3.2 引入了**滑動視窗 (Sliding Window)** 機制。

### 核心邏輯
- **歷史緩衝**: `ClaudeTranslator` 會保留最近 3 句的翻譯紀錄（原文與譯文）。
- **Prompt 注入**: 每次請求翻譯時，會將歷史紀錄作為 context 發送給 Claude。
- **重置時機**: 當用戶停止錄音、切換語言或清除快取時，歷史紀錄會被重置，以防止不同主題間的上下文干擾。

### 翻譯觸發機制（效能與成本平衡）
- **Final 觸發**: 為了確保翻譯質量與減少 API 成本，系統**僅對 Final 結果**進行完整翻譯。
- **Interim 處理**: Interim 字幕僅顯示原文，不觸發翻譯。這樣可以避免字幕因為 AI 翻譯延遲而產生的跳動與重複消耗 Token。

---

## 🎯 術語表 (Glossary) 處理

系統支援使用者自定義「術語表」，這對於技術文件、專有名詞或特定對話背景非常重要。

### 解析規則
- **語法**: 支援 `Key: Value`, `Key = Value`, `Key -> Value` 等多種格式。
- **動態注入**: 術語表會被轉換成系統提示語（System Prompt），例如：
  > "You MUST use the following specific translations: PR -> 拉取請求, CI -> 持續整合"
- **優先權**: 術語表中的對照關係具有最高優先權，AI 會強制遵守。
...
✅ **改進**：
- **Claude 4.5 升級**：使用更快速且聰明的 Haiku 4.5 模型。
- **3 句滑動視窗**：解決代名詞（It, This, They）翻譯不明確的問題。
- **使用者術語表**：讓用戶能自定義專業名詞，解決 AI 亂翻譯術語的問題。
- **Final 翻譯策略**：僅翻譯最終結果，大幅降低 API 支出並提升 UI 穩定性。

✅ **效果**：
- 翻譯內容更符合對話邏輯，連貫性提升。
- 專業術語翻譯 100% 精準（基於用戶字典）。
- 解決了 Interim 期間字幕頻繁跳動的問題。

📝 **相關提交**: `feature/claude-4.5-upgrade-and-glossary`

...
## 2026-03-08 - 上下文感知翻譯與術語表（Phase 3.2 完成）

### 🎯 Phase 3.2 目標
提升翻譯的專業性與連貫性，優化台灣常見的雙語混合辨識。

**成就**：
- ✅ **Claude 4.5 升級**：實作 `claude-haiku-4-5` 模型與 3 句滑動視窗歷史，顯著提升代名詞與上下文翻譯的一致性。
- ✅ **術語表 (Glossary) 功能**：新增使用者字典，支援強制 AI 遵守專業名詞對照。
- ✅ **Deepgram 混合辨識優化**：針對台灣口語習慣，採用 `language=multi` 搭配具體語言提示（zh-TW, en），解決代碼切換辨識問題。
- ✅ **效能優化**：改為僅翻譯 `isFinal: true` 的結果，降低 API 成本並消除字幕顯示抖動。

---

### 🐛 問題 1: 翻譯結果缺乏連貫性（Pronoun Issues）

**症狀**:
- 當說話者提到 "it" 或 "this" 時，AI 經常翻譯錯誤或不符合前文語境。
- 翻譯結果在句子與句子之間缺乏一致性。

**解決方案**:
在 `ClaudeTranslator.js` 中實作一個 3 句長度的歷史緩衝區（Sliding Window）。每次翻譯時會將前兩句原文與譯文連同當前句子一起發送給 AI，讓 AI 了解對話背景。

---

### 🐛 問題 2: 專業術語被 AI「過度翻譯」或翻譯不一

**症狀**:
- 像 "PR"、"CI/CD" 等術語有時被翻成中文，有時保留英文。
- 用戶希望在特定專案中固定某些術語的譯名。

**解決方案**:
實作 **User Glossary** 功能。用戶可在 Popup 中定義 `Key: Value` 對照表，程式會解析並動態注入 Claude 的 System Prompt 中，強制 AI 遵守譯名規範。

---

### 🐛 問題 3: Deepgram 混合辨識語法錯誤

**症狀**:
- 之前的實作嘗試重複發送 `language` 參數，導致 API 返回錯誤。
- 在 `zh-TW` 與 `en` 之間切換時，辨識結果不穩定。

**解決方案**:
優化 `DeepgramClient` 的 URL 生成邏輯。現在使用 `language=multi` 作為核心模式，並透過 `URLSearchParams.append()` 同時附加 `zh-TW` 與 `en` 作為 Hints。這能最有效地觸發 Nova-2/3 的 Code-switching 功能。

---

## 2025-12-22 - Claude API 集成（Phase 3.1 完成）
...
**選擇的模型**：Claude 4.5 Haiku
- 成本低：$1.00/$5.00 per 1M tokens (input/output)
- 速度快：優先考慮延遲
- 質量：良好（80-90% 準確度）
...
|------|----------|-----------|
| **成本** | $1.00/$5.00 per 1M | $3/$15 per 1M |
